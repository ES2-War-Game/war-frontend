  ...
  persist(
    (set) => ({
      ...
      setAllocate: (amount = 1) =>
        set((s) => ({
          unallocatedArmies: Math.max(
            0,
            s.unallocatedArmies - Math.max(1, amount)
          ),
        })),
      ...
      setDesallocate: (amount = 1) =>
        set((s) => ({
          unallocatedArmies: s.unallocatedArmies + Math.max(1, amount),
        })),

      setUnallocatedArmies: (status: number) =>
        set(() => ({ unallocatedArmies: status })),
    }),
    {
      name: "allocate-store",
      partialize: (state) => ({
        allocating: state.allocating,
        unallocatedArmies: state.unallocatedArmies,
      }),
    }
  )
);
